...
select max(salary*months), count(name) from employee group by (salary*months) order by (salary*months) desc limit 1;

--https://www.hackerrank.com/challenges/the-report/problem?isFullScreen=true
select IF(Grade<8, null, Name), Grade, Marks from Students join Grades where Marks between Min_Mark and Max_Mark order by Grade desc, Name, Marks;
...
select a.name, b.name as FriendName from students a, students b, friends f where a.id=f.id and b.id = f.friend_id;